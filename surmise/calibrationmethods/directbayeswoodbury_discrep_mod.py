...
def fit(fitinfo, emu, x, y,  args=None):
    ...
    emu_bias = bias_fit(fitinfo, emu, x, y)
...
def predict(predinfo, fitinfo, emu, x, args=None):
    ...
    emubias = emu_bias.predict(theta=x)
    emupredict = emu.predict(x, theta)
    predinfo['rnd'] = copy.deepcopy(emubias().T + emupredict()).T
    ...
    predinfo['var'] = np.mean(varfull, 0) + np.var(emumean, 0) + emubias.var().T
    return
...
def loglik(fitinfo, emu, emu_bias, theta, y, x, args):
    r"""
    ...
    if np.any(np.abs(emuvar/(10 ** (-4) +
        ...
        emuoldvar = emuoldpredict.var() + emubias.var().T
...
def loglik_grad(fitinfo, emu, emu_bias, theta, y, x, args):
    r"""
    This is a optional docstring for an internal function.
    """
    ...
    if np.any(np.abs(emuvar/(10 ** (-4) +
                             (1 + 10**(-4))*np.sum(np.square(emucovxhalf),
                                                   2))) > 1):
        emuoldpredict = emu.predict(x) + emubias.mean().T
        emuoldvar = emuoldpredict.var() + emubias.var().T
        emuoldcxh = emuoldpredict.covxhalf()
        obsvar += np.mean(np.abs(emuoldvar -
                                 np.sum(np.square(emuoldcxh), 2)), 1)
...
def bias_fit(fitinfo, emu, x, y, theta0=None):
    thetaprior = fitinfo['thetaprior']
    if theta0 is None:
        theta = np.vstack((thetaprior.rnd(1000), copy.copy(emu._emulator__theta)))
        abserr0 = (np.abs(y.reshape((len(y), 1)) - emu.predict(x, theta).mean())).mean(0)

        theta0 = theta[np.argmin(abserr0)]

    def obj(theta, emu, x, y):
    ...
    thetap = opval.x
    biasp = y.reshape((len(y), 1)) - emu.predict(x, thetap).mean()

    emu_bias = emulator(x=np.ones((1, 1)), theta=x, f=biasp)

    fitinfo['bias'] = emu_bias
    return emu_bias
